...
$score = 0 + (1 * $user['posts']) + (15 * $user['postplusones']) - (10 * $deletedposts);
$lives = 0 + (0.01 * $user['posts']) + (0.15 * $user['postplusones']);
$karma = 0 + (1 * $user['postplusones']) - (20 * $deletedposts);
...
if($user['status'])
{
	$status = preg_replace('@<br.*?>\s*(\S)@i', ' &bull; $1', strip_tags(CleanUpPost($user['status'], "", true), "<b><strong><i><em><span><s><del><img><a><br><br/><small>"));
}
