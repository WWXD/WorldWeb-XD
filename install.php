...
<html>
	<head>
		<title>Blargboard XD Installation</title>
		<style>
	...
	if (file_exists(__DIR__.'/config/database.php'))
		die($header.'The board is already installed.'.$footer);
	...
	if ($_POST['submit'])
	{
		...
		if (!$boardusername || !$boardpassword)
			die($header.'Please enter a board username and password.'.$footer);
		...
		Query("insert into {users} (id, name, password, pss, primarygroup, regdate, lastactivity, lastip, email, sex, theme) values ({0}, {1}, {2}, {3}, {4}, {5}, {5}, {6}, {7}, {8}, {9})", 
			1, $boardusername, $sha, $pss, 4, time(), $_SERVER['REMOTE_ADDR'], '', 2, 'stone');
				...
				<form action="" method="POST">
					<div class="outline">
						...
						<div class="box cell center">
							...
							<small>Change this if the board's database is shared with other applications. Leaving this blank is fine.</small>
						</div>
						<div class="box col2 center">
							Owner Credentials
						</div>
